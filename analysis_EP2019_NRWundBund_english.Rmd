...
#Auswahl der Parteien in EP14 und EP19
selection_partiesinEP14and19 <- c("CDU","SPD","GRÜNE","DIE LINKE","AfD","FDP","FREIE WÄHLER","PIRATEN","Tierschutzpartei","NPD","FAMILIE","ÖDP","Die PARTEI","Volt")
...
deu_ep19_data_long %>% 
  filter(gehort_zu == 5) %>%
  filter(Kurzbezeichnung %in% selection_partiesinEP14and19) %>% 
ggplot() +
  geom_smooth(aes(x = turnout, y = value), method = "lm", se = TRUE)+
  facet_wrap(~ Kurzbezeichnung, scales = "free_y") +
  labs(title = "Relationship between voter turnout and party result",
       subtitle = "level of analysis: counties and autonomous cities",
       x = "voter turnout 2019 (%)",
       y = "party result 2019 (%)")
ggsave(filename = "gfx/E/03_wb_parteiergebnis_NRW_eng.png", plot = last_plot(), device = "png", dpi = 400, width = 11, height = 7)
...
indicator_labs_epelections <- c(alo_quote = "unemployment rate (%)", 
                    alter_75plus = "share of citizens aged 75+ (%)",
                    bev_ausl = "share of foreign population (%)",
                    edu_absolventen_abi = "Share of graduates with 'Abitur' (in that year) (%)")
...
deu_ep_all_long %>% 
  ...
  facet_wrap(~variable, scales = "free_x", labeller = as_labeller(indicator_labs_epelections))+
  scale_color_discrete(name = "", labels = c("EP 2019", "EP 2014"))+
  labs(title = "Relationship between socioeconomic indicators and voter turnout in NRW",
       subtitle = "level of analysis: counties and autonomous cities",
       x = "Social and economic indicators",
       y = "voter turnout (%)")+
  theme_bw()
ggsave(filename = "gfx/E/07_wb_soceco_EPelec_NRW_eng.png", plot = last_plot(), device = "png", dpi = 400, width = 11, height = 7)
